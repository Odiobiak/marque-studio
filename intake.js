...
(function () {
  ...
  function render() {
    var r = calc();
    if (rangeEl) rangeEl.textContent = money(r.low) + ' to ' + money(r.high);
    if (monthlyEl) {
  ...
  function buildBrief() {
    ...
    L.push('Estimate:  ' + money(r.low) + ' to ' + money(r.high) + (r.monthly > 0 ? ' + ' + money(r.monthly) + '/mo' : ''));
    return L.join('\n');
  }
  ...
  form.addEventListener('submit', function (e) {
    e.preventDefault();
    if (!validate()) return;
    var subject = 'Project brief from ' + (val('name') || 'a new enquiry');
    var body = buildBrief();
